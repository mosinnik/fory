...
public class LongStringSerializationSuite {
  ...
  @Benchmark
  public Object fory_serializeLongStr(ForyState.DataState state) {
    state.buffer.writerIndex(0);
    state.fory.serialize(state.buffer, state.data.longStr);
    return state.buffer;
  }
  ...
  @Benchmark
  public byte[] onennio_serializeLongStr(OneNioState.DataState state) {
    return OneNioState.serialize(state.data.longStr);
  }
